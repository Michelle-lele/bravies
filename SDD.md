...
- **Resolved, after two follow-up bug fixes**: Wait-list Signup's clouds are now bigger AND allowed to bleed over the FAQ section above (both direct requests), using a `--cloud-y-offset` custom property that stays correct regardless of section height or viewport width. Landed two real bugs along the way that needed separate fixes: a page-wide horizontal scrollbar (root cause: `<html>` didn't have `overflow-x: hidden`, only `<body>` did — only became load-bearing once cloud clipping was removed) and actual text overlap with the FAQ's last question (confirmed via measured bounding-box overlap, not just visual inspection) — fixed with coordinated cloud-size and `.faq` padding-bottom adjustments, re-verified clear at five different widths.
