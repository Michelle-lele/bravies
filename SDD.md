...
- **Phase 4.5 (revised in the interim round below)**: an eyebrow badge sits above the heading (visual order: badge → heading → form). Originally shipped as one long pill wrapping the entire sentence "Ексклузивен достъп преди всички и специална цена за първите записали се." — revised per direct feedback into two separate short chips, `.waitlist-signup__badge` (one per highlighted phrase, "Ексклузивен достъп" / "Специална цена"), with the connecting words as plain white text on the yellow background rather than inside any chip. No new component pattern introduced: chips still reuse `var(--radius-input)`, the same corner radius already used by inputs/buttons/the success message elsewhere on the page, and `--color-zaki` — already this exact section's own accent color since Phase 4 (the submit button) — on a white background.
  - **Responsive line-break, by direct request**: one line at desktop/landscape-tablet, two stacked rows (one chip per row) at mobile/portrait-tablet, breaking exactly after "и". Implemented by wrapping each chip + its trailing connector text in its own `.waitlist-signup__badge-line` span — `display: inline` by default (both flow together as one paragraph), switched to `display: block` below the row-layout breakpoint (written as the direct "narrow" condition — mobile of any orientation, OR portrait tablet — rather than negating the row-layout media query with `not`, which needs De Morgan's-style unpacking to stay correct for a comma-separated/AND-compound condition like this one). No explicit `<br>` needed; the block-level break falls exactly at the span boundary, which is exactly where the copy wants it.
  - **A real sizing bug surfaced during verification, not just a media-query tweak**: even after the line-break logic above was correct, the badge was still visibly wrapping at desktop — turned out its natural width (~570px, both chips plus all connecting text) exceeds `.waitlist-signup__inner`'s own `480px` max-width, so the browser was wrapping at whatever space ran out of room first, not at a boundary the `display: inline`/`block` toggle controlled. Fixed with `white-space: nowrap` on the badge wrapper specifically at the row-layout breakpoint, which removes every line-break opportunity in the paragraph and lets it render wider than the 480px column around it (no `overflow` rule stops this) — the heading and form below stay at their existing 480px-constrained width; only this one line intentionally spans wider. Verified one line at exactly 1440px and 1024px after this fix, not just visually skimmed.
